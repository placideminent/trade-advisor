...
WEIGHT_FIELDS = [
    ...
    ("chg6_100", "6개월 상승 100%", "6개월 전 대비 100% 이상 250% 미만 −1 (모든 조회)"),
    ("chg6_250", "6개월 상승 250%", "6개월 전 대비 250% 이상 350% 미만 −2 (모든 조회)"),
    ("chg6_350", "6개월 상승 350%", "6개월 전 대비 350% 이상 −3 (모든 조회)"),
...
def recommend(
    ...
    **_unused,
    ...
    if chg6 is None:
        ...
        add("6개월 상승률", f"{chg6 * 100:.1f}% (350% 이상)", wp("chg6_350"))
    elif chg6 >= 2.5:
        add("6개월 상승률", f"{chg6 * 100:.1f}% (250% 이상 350% 미만)", wp("chg6_250"))
    elif chg6 >= 1.0:
        add("6개월 상승률", f"{chg6 * 100:.1f}% (100% 이상 250% 미만)", wp("chg6_100"))
    else:
