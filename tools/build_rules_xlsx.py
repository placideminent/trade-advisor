...
def main() -> None:
    ...
    add_sheet(
        ...
        [
            ...
            ["상승 추세선 근접", "현재가가 상승 추세선을 완전 이탈했을 때", -1],
            ...
            ["밸류 상단 (VAH)", "현재가가 비싼 구간 인데", -1],
            ["RSI", "30 이하 (너무 많이 떨어짐)", 1],
            ["RSI", "70 이상 (너무 많이 오름)", -1],
            ["RSI", "30~70 사이", 0],
            ["20일선", "현재가가 20일선보다 아래이고, 상승 추세일때", 1],
            ["20일선", "현재가가 20일선보다 아래이고, 하락 추세일때", -1],
            ["180일선", "현재가가 180일선 아래위로 근처일 때(6개월 수익률용으로 일봉을 220일까지 받고\n• 주말·휴장이면 다음 거래일 종가로 계산합니다.)", 1],
            ["1개월 상승률", "한 달 동안 30% 이상 오름", -1],
            ["1개월 하락률", "한 달 동안 20% 이상 떨어짐", 1],
            ["6개월 상승률", "6개월 동안 600% 이상오름", -3],
            ["6개월 상승률", "6개월 동안 100% 이상오름", -2],
            ["6개월 상승률", "6개월 동안 50% 이상 100% 미만 오름", -1],
