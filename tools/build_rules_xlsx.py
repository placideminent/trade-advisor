...
def main() -> None:
    ...
    add_sheet(
        ...
        [
            ...
            ["하방향 1개월 추세선 고려", "(3개월,6개월,1년 조회에만 적용) 하락 추세선 상승 추세선 모두 하락이면서 해당 시점에 1개월(1시간봉기준)조회시 상승추세선이 상방향일 때, 상방향 아니면 무효", 1],
            ["상방향 1개월 추세선 고려", "(3개월,6개월,1년 조회에만 적용) 하락 추세선 상승 추세선 모두 상승이면서 해당 시점에 1개월(1시간봉기준)조회시 상승추세선이 하방향일 때, 하방향 아니면 무효", 1],
            ...
            ["신고가", "위에 저항이 아예 없는 신고가의 경우 +1점, 신고가 이후 4개봉 지나면 무효", 1],
            ["신고가 돌파 실패", "신고가 이후 8개봉동안 신고가 갱신 못하면 -1점, 신고가 이후 12개봉 지나면 무효", -1],
        ],
        [18, 72, 10],
    )
