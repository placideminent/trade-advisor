...
def main() -> None:
    ...
    add_sheet(
        ...
        [
            ...
            ["추세", "횡보일 때", 0],
            ["스윙 저점 근접", "현재가가 조회 기간 스윙 저점 근처일 때", 1],
            ["스윙 고점 근접", "현재가가 조회 기간 스윙 고점 근처일 때", -1],
            ["하락 추세선 근접", "하락 추세선에 근접 했을 때", -1],
            ["추세선 방향", "하락 추세선 상승 추세선 모두 하락일 때", -1],
            ["추세선 방향", "6개월,1년 조회시 하락 추세선 상승 추세선 모두 상승이면서 1개월(1시간봉) 조회시 하락일때", 1],
            ["상승 추세선 이탈", "현재가가 상승 추세선을 완전 이탈했을 때, 이탈후 최근봉 4봉이 지났으면 무효", -1],
            ...
            ["1개월 하락률", "한 달 동안 40% 이상 떨어짐", 2],
            ["6개월 상승률", "6개월 동안 600% 이상 오름", -2],
            ["6개월 상승률", "6개월 동안 200% 이상 600% 미만 오름", -1],
