...
def main() -> None:
    ...
    how_lines = [
        ...
        "",
        "순서",
        "1. 「기본 점수」부터 「신고가 이탈」까지 더합니다.",
        "2. 더한 점수를 %로 바꿉니다. 0점이하면 0%, 12점(기본)이면 50%, 24점이상이면 100%.",
        "3. %로 매수 / 매도 / 홀딩을 정합니다.",
        ...
        "1개월 조회 상승선은 마지막 스윙 저점 두 개, 6개월(3개월·1년) 상승선은 중간 저점이 안 깨는 상승 지지선입니다.",
    ]
    ...
    add_sheet(
        ...
        [
            ["기본", "시작할 때 항상 줌", 12],
            ["조회기간 추세", "(3개월,6개월,1년 조회에만 적용)조회기간 추세가 상승이면서 1개월 조회시(1시간봉) 상승 추세일때", -1],
            ["조회기간 추세", "(3개월,6개월,1년 조회에만 적용)조회가간 추세가 상승이면서 1개월 조회시(1시간봉) 하락 추세일때", 1],
            ["조회기간 추세", "(3개월,6개월,1년 조회에만 적용)조회기간 추세가 하락이면서 1개월 조회시(1시간봉) 상승 추세일 때", -1],
            ["조회기간 추세", "(3개월,6개월,1년 조회에만 적용)조회기간 추세가 하락이면서 1개월 조회시(1시간봉) 하락 추세일때", 1],
            ["조회기간 추세", "(3개월,6개월,1년 조회에만 적용)조회기간 추세가 횡보이면서 1개월 조회시(1시간봉) 하락 추세일때", 1],
            ["조회기간 추세", "(3개월,6개월,1년 조회에만 적용)조회기간 추세가 횡보이면서 1개월 조회시(1시간봉) 상승 추세일 때", 0],
            ["조회기간 추세", "(3개월,6개월,1년 조회에만 적용)조회기간 추세와 1개월 조회시(1시간봉) 상승 추세일 때", 0],
            ...
            ["하방향 1개월 추세선 고려", "(3개월,6개월,1년 조회에만 적용) 하락 추세선 상승 추세선 모두 하락이면서 해당 시점에 1개월(1시간봉기준)조회시 상승추세선이 상방향일 때", 1],
            ["상방향 1개월 추세선 고려", "(3개월,6개월,1년 조회에만 적용) 하락 추세선 상승 추세선 모두 상승이면서 해당 시점에 1개월(1시간봉기준)조회시 상승추세선이 하방향일 때 상승 추세선에 근접했으면 가점, 하방향 아니면 무효", 1],
    ...
    cuts["A11"] = "점수를 %로 바꾸는 법: 0점이하면 0%, 12점(기본)이면 50%, 24점이상이면 100%."
    cuts["A11"].font = Font(italic=True, color="666666")
    cuts.merge_cells("A11:E11")
    cuts["A12"] = "가까운 가격: 하루 변동폭의 약 30%, 또는 주가의 1.0% 중 더 큰 값."
